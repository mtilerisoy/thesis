...
def epoch_wrapup(pl_module):
    ...
    for loss_name, v in pl_module.hparams.config["loss_names"].items():
        ...
        if loss_name == "vqa":
            value = getattr(pl_module.student_model, f"{phase}_{loss_name}_score").compute()
            pl_module.log(f"{loss_name}/{phase}/score_epoch", value)
            getattr(pl_module.student_model, f"{phase}_{loss_name}_score").reset()
            pl_module.log(
                f"{loss_name}/{phase}/loss_epoch",
                getattr(pl_module.student_model, f"{phase}_{loss_name}_loss").compute(),
            )
            getattr(pl_module.student_model, f"{phase}_{loss_name}_loss").reset()
        elif loss_name == "nlvr2":
            if phase == "train":
                value = getattr(pl_module.student_model, f"train_{loss_name}_accuracy").compute()
                pl_module.log(f"{loss_name}/train/accuracy_epoch", value)
                getattr(pl_module.student_model, f"train_{loss_name}_accuracy").reset()
                pl_module.log(
                    f"{loss_name}/train/loss_epoch",
                    getattr(pl_module.student_model, f"train_{loss_name}_loss").compute(),
                )
                getattr(pl_module.student_model, f"train_{loss_name}_loss").reset()
            else:
                value = getattr(pl_module.student_model, f"dev_{loss_name}_accuracy").compute()
                pl_module.log(f"{loss_name}/dev/accuracy_epoch", value)
                getattr(pl_module.student_model, f"dev_{loss_name}_accuracy").reset()
                pl_module.log(
                    f"{loss_name}/dev/loss_epoch",
                    getattr(pl_module.student_model, f"dev_{loss_name}_loss").compute(),
                )
                getattr(pl_module.student_model, f"dev_{loss_name}_loss").reset()

                value = getattr(pl_module.student_model, f"test_{loss_name}_accuracy").compute()
                pl_module.log(f"{loss_name}/test/accuracy_epoch", value)
                getattr(pl_module.student_model, f"test_{loss_name}_accuracy").reset()
                pl_module.log(
                    f"{loss_name}/test/loss_epoch",
                    getattr(pl_module.student_model, f"test_{loss_name}_loss").compute(),
                )
                getattr(pl_module.student_model, f"test_{loss_name}_loss").reset()
        elif loss_name == "irtr":
            pl_module.log(
                f"{loss_name}/{phase}/irtr_loss_epoch",
                getattr(pl_module.student_model, f"{phase}_irtr_loss").compute(),
            )
            getattr(pl_module.student_model, f"{phase}_irtr_loss").reset()
            ...
            value = getattr(pl_module.student_model, f"{phase}_{loss_name}_accuracy").compute()
            pl_module.log(f"{loss_name}/{phase}/accuracy_epoch", value)
            getattr(pl_module.student_model, f"{phase}_{loss_name}_accuracy").reset()
            pl_module.log(
                f"{loss_name}/{phase}/loss_epoch",
                getattr(pl_module.student_model, f"{phase}_{loss_name}_loss").compute(),
            )
            ...
            pl_module.log(
                ...
                getattr(pl_module.student_model, f"{phase}_{loss_name}_wpa_loss").compute(),
            )
            getattr(pl_module.student_model, f"{phase}_{loss_name}_wpa_loss").reset()
        else:
            value = getattr(pl_module.student_model, f"{phase}_{loss_name}_accuracy").compute()
            pl_module.log(f"{loss_name}/{phase}/accuracy_epoch", value)
            getattr(pl_module.student_model, f"{phase}_{loss_name}_accuracy").reset()
